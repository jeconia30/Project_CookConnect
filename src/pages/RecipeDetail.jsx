...
const RecipeDetail = () => {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      await api.post(`/comments`, { recipe_id: id, content: commentInput }, {
          headers: { Authorization: `Bearer ${authToken}` }
      });
  ...
  const handleFollow = () => {
    if(!recipe) return;
    const endpoint = isFollowing ? `/users/${recipe.username}/unfollow` : `/users/${recipe.username}/follow`;
  ...
  if (error || !recipe) {
    return (
      <div className="feed-area" style={{ textAlign: "center", marginTop: "50px", padding: "20px" }}>
        <h2>🚫 Oops!</h2>
        <p>{error || "Resep tidak ditemukan."}</p>
        <button className="cta-button" onClick={() => navigate('/feed')} style={{marginTop: '20px'}}>
          Kembali ke Feed
        </button>
      </div>
    );
        ...
        <div className="recipe-detail-container">
          ...
          <div className="recipe-detail-header">
            ...
            <div className="post-header revised">
              <div className="profile-pic-small" style={{ backgroundImage: `url(${recipe.avatar_url})` }}></div>
              <div className="user-details-column">
                <div className="user-details-top">
                  <strong>{recipe.user_fullname}</strong>
                  <span className="username">@{recipe.username}</span>
                  <span className="dot-separator">•</span>
                  <button className="follow-button text-only" onClick={handleFollow}>
                    {isFollowing ? "Mengikuti" : "Ikuti"}
                  </button>
                </div>
                <span className="post-meta">{new Date(recipe.created_at).toLocaleDateString()}</span>
              </div>
            </div>
          </div>

          {/* Image */}
          <img src={recipe.image_url} alt={recipe.title} className="recipe-hero-image" />

          {/* Actions */}
          <div className="post-actions detail-page-actions">
            <button onClick={handleLike} className={isLiked ? "like-active" : ""}>
              <i className={isLiked ? "fas fa-heart" : "far fa-heart"}></i> {formatNumber(likeCount)} Suka
            </button>
            <a href="#comments-section" style={{ textDecoration: "none", color: "inherit" }}>
              <i className="far fa-comment"></i> {comments.length} Komentar
            </a>
            <button onClick={handleShare}><i className="far fa-share-square"></i> Bagikan</button>
            <button onClick={handleSave} className={isSaved ? "save-active" : ""}>
              <i className={isSaved ? "fas fa-bookmark" : "far fa-bookmark"}></i> {isSaved ? "Disimpan" : "Simpan"}
            </button>
          </div>
